...
export default function Quiz({ session }) {
    ...
    const handlePrev = () => {
        if (mode === 'mistakes') {
            const currentPos = mistakeIndices.indexOf(currentIndex);
            if (currentPos > 0) {
                setCurrentIndex(mistakeIndices[currentPos - 1]);
            }
            return;
        }

        // Standard Mode: Find previous non-excluded index
    ...
    const handleNext = () => {
        if (mode === 'mistakes') {
            const currentPos = mistakeIndices.indexOf(currentIndex);
            if (currentPos !== -1 && currentPos < mistakeIndices.length - 1) {
                setCurrentIndex(mistakeIndices[currentPos + 1]);
            }
            return;
        }

        // Standard Mode: Find next non-excluded index
        let newIndex = currentIndex + 1;
        const excludedJson = localStorage.getItem('quiz_excludedIndices');
        const excludedIndices = excludedJson ? JSON.parse(excludedJson) : [];
        ...
        <div className="app-container">
            ...
            <footer>
                <div className="footer-content">
                    <button className="nav-btn" onClick={handlePrev} disabled={
                        mode === 'mistakes'
                            ? (mistakeIndices.indexOf(currentIndex) <= 0)
                            : (currentIndex === 0)
                    }>Previous</button>
                    ...
                    <button className="nav-btn" onClick={handleNext} disabled={
                        mode === 'mistakes'
                            ? (mistakeIndices.indexOf(currentIndex) === -1 || mistakeIndices.indexOf(currentIndex) >= mistakeIndices.length - 1)
